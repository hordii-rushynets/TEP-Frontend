...
export function TotalPriceBlock({
  ...
  const totalPrice = +(goods.reduce((acc, el) => acc + (el?.product_variants?.promotion ? el?.product_variants?.promo_price : el?.product_variants?.default_price) * el.quantity, 0)).toFixed(2);
